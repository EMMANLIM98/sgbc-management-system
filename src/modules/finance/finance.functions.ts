...
export const getFinanceKpis = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const expenses_delta_pct =
      expenses_prev > 0 ? Math.round(((expenses_mtd - expenses_prev) / expenses_prev) * 100) : null;
...
export const getRecentFinanceActivity = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let cq = sb
      .from("contributions")
      .select(
        "id, amount, currency, occurred_on, note, created_at, churches(name), finance_categories(name), members(first_name,last_name)",
      )
    ...
    let eq = sb
      .from("expenses")
      .select(
        "id, amount, currency, occurred_on, payee, note, created_at, churches(name), finance_categories(name)",
      )
      .order("occurred_on", { ascending: false })
      .order("created_at", { ascending: false })
      .limit(data.limit);
...
export const getFinanceByChurch = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return (churches ?? []).map((ch) => ({
      ...
      giving_mtd: giving[ch.id] ?? 0,
      expenses_mtd: spend[ch.id] ?? 0,
