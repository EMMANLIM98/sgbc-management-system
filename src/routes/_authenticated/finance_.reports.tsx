  ...
  async function onExportPdf() {
    ...
    if (report.by_church.length) {
      autoTable(doc, {
        ...
        body: report.by_church.map((r) => [
          r.name,
          formatPHP(r.giving),
          formatPHP(r.expenses),
          formatPHP(r.net),
        ]),
        theme: "grid",
        styles: { fontSize: 9, cellPadding: 5 },
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Shekinah Glory Baptist Church  ·  ${periodLabel}  ·  ${scopeLabel}`,
        marginX,
    ...
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        ...
        <KPI
          icon={Wallet}
          label="Net"
          value={formatPHP(report?.net_total)}
          sub={(report?.net_total ?? 0) >= 0 ? "surplus" : "deficit"}
        />
        <KPI
          icon={TrendingUp}
          label="Transactions"
          value={`${(report?.transaction_counts.giving ?? 0) + (report?.transaction_counts.expenses ?? 0)}`}
              ...
              <tbody>
                {report!.by_church.map((r) => (
                  <tr key={r.name} className="border-b border-border last:border-0">
                    ...
                    <td
                      className={`px-4 py-2 text-right tabular font-medium ${r.net >= 0 ? "" : "text-destructive"}`}
                    >
                      {formatPHP(r.net)}
                    </td>
                  </tr>
            ...
            <tbody>
              {(report?.trend ?? []).map((t) => (
                <tr key={t.key} className="border-b border-border last:border-0">
                  ...
                  <td
                    className={`px-4 py-2 text-right tabular font-medium ${t.giving - t.expenses >= 0 ? "" : "text-destructive"}`}
                  >
                    {formatPHP(t.giving - t.expenses)}
                  </td>
                </tr>
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.name} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-2 text-right tabular whitespace-nowrap">
                    {formatPHP(r.amount)}
                  </td>
                </tr>
              );
