...
import {
  getFinanceKpis, getGivingTrend, getGivingByCategory,
  getRecentFinanceActivity, getFinanceByChurch,
...
function FinancePage() {
  ...
  const { data: kpis } = useQuery({ queryKey: ["fin-kpis", currentChurchId], queryFn: () => kpisFn({ data: scope }) });
  const { data: trend } = useQuery({ queryKey: ["fin-trend", currentChurchId], queryFn: () => trendFn({ data: { ...scope, months: 6 } }) });
  const { data: cats } = useQuery({ queryKey: ["fin-cats", currentChurchId], queryFn: () => catFn({ data: scope }) });
  const { data: activity } = useQuery({ queryKey: ["fin-act", currentChurchId], queryFn: () => actFn({ data: { ...scope, limit: 12 } }) });
  const { data: byChurch } = useQuery({ queryKey: ["fin-by-church"], queryFn: () => byChurchFn(), enabled: !currentChurchId });
    ...
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={trend ?? []} margin={{ left: -10, right: 8, top: 8, bottom: 0 }}>
                ...
                <Bar dataKey="giving" name="Giving" fill="oklch(0.62 0.15 158)" radius={[3, 3, 0, 0]} />
                <Bar dataKey="expenses" name="Expenses" fill="oklch(0.65 0.18 25)" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="border border-border rounded-lg bg-card p-4">
          ...
          {(cats ?? []).length === 0 ? (
            <div className="text-[12px] text-muted-foreground py-10 text-center">No giving recorded this month.</div>
          ) : (
              ...
              <tbody>
                {byChurch.map((c) => (
                  <tr key={c.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-2 text-right tabular text-muted-foreground">{formatPHP(c.expenses_mtd)}</td>
                    <td className={`px-4 py-2 text-right tabular font-medium ${c.net_mtd >= 0 ? "" : "text-destructive"}`}>
                      {formatPHP(c.net_mtd)}
                    </td>
                  </tr>
                ))}
      ...
      <div className="border border-border rounded-lg bg-card">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          ...
          <Link to="/finance" className="text-[11px] text-muted-foreground inline-flex items-center gap-1 hover:text-foreground">
        ...
        <ul className="divide-y divide-border">
          {(activity ?? []).map((a) => (
            <li key={a.id} className="px-4 py-3 flex items-start gap-3 text-[13px]">
              <div className={`h-7 w-7 mt-0.5 rounded-full grid place-items-center ${
                a.kind === "contribution" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" : "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300"
              }`}>
                {a.kind === "contribution" ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="truncate">
                  <span className="font-medium">{a.kind === "contribution" ? "Giving" : "Expense"}</span>
              ...
              <div className={`tabular text-[13px] font-medium whitespace-nowrap ${
                a.kind === "contribution" ? "" : "text-muted-foreground"
              }`}>
                {a.kind === "expense" ? "−" : "+"}{formatPHP(a.amount)}
              </div>
            </li>
          ))}
          {(activity ?? []).length === 0 && (
...
function KPI({ icon: Icon, label, value, sub, delta, invertDelta }: {
  ...
  return (
    <div className="border border-border rounded-lg bg-card p-4">
      ...
      <div className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-1.5">
        {showDelta && (
          <span className={`inline-flex items-center gap-0.5 font-medium ${positive ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
