...
export const Route = createFileRoute("/_authenticated/finance_/members/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Complete contribution history with breakdown by giving type and receipts." },
    ],
  }),
...
function MemberHistoryPage() {
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      <div className="mb-2">
        <Link to="/finance/contributions" className="text-[12px] text-muted-foreground inline-flex items-center gap-1 hover:text-foreground">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link to="/members/$id" params={{ id }} className="inline-flex items-center gap-1.5 h-8 px-3 text-[12px] rounded-md border border-border hover:bg-accent">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <div className="border border-border rounded-lg bg-card p-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Lifetime giving</div>
        ...
        <div className="border border-border rounded-lg bg-card p-4 md:col-span-2">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-2">Breakdown by giving type</div>
                ...
                return (
                  <li key={c.id}>
                    <div className="flex items-center justify-between text-[12px] mb-1">
                      ...
                      <span className="tabular">{formatPHP(c.total)} <span className="text-muted-foreground">({pct}%)</span></span>
                    </div>
      ...
      <div className="border border-border rounded-lg bg-card">
        <div className="px-4 py-3 border-b border-border text-[13px] font-medium">All contributions</div>
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody>
                ...
                <tr key={r.id} className="border-b border-border last:border-0 hover:bg-accent/40">
                  ...
                  <td className="px-4 py-2 text-right">
                    <button
                      onClick={async () => {
                        try {
                          await downloadReceiptPdf({ ...r, amount: Number(r.amount), members: data?.member ?? null });
                        } catch (e: any) {
              ...
              {!isLoading && (data?.rows ?? []).length === 0 && (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-muted-foreground">No contributions on record.</td></tr>
              )}
            </tbody>
          </table>
        </div>
