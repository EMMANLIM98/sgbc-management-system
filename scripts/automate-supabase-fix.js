...
async function fixSupabase() {
  ...
  try {
    ...
    const newQueryButton = page.locator(
      'button:has-text("New query"), [role="button"]:has-text("New"), button[aria-label*="New"]',
    );
    ...
    const editorArea = page
      .locator('[class*="editor"], [class*="monaco"], textarea, [contenteditable="true"]')
      .first();
    ...
    const runButton = page.locator(
      'button:has-text("Run"), [role="button"]:has-text("Execute"), button[title*="Run"]',
    );
    ...
    const successMessage = page
      .locator('text=successfully, text=completed, [class*="success"]')
      .first();
